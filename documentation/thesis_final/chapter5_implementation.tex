...
The model was trained using AdamW with weight decay $0.05$. Differential learning rates were used: $1\times10^{-5}$ for the pretrained CNN backbone and $1\times10^{-4}$ for the remaining network parameters. A 5-epoch linear warmup followed by cosine decay was used over a maximum of 100 epochs. Gradient clipping with max norm 5.0 and early stopping with patience 15 were applied for stability.
...
\begin{itemize}
    ...
    \item Objective balancing contributed more to stability gains than simply scaling model size in the current compute regime.
...
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Analytical Component} & \textbf{Development Validation} & \textbf{Final Test Array} \\
\midrule
...
The achieved test WER of 50.91\% demonstrated that the proposed hybrid objective provided stable training behavior and competitive baseline performance under constrained hardware and limited-data conditions. In this thesis, 51.44\% refers to the best development-side checkpoint from the ResNet-18 branch, while 50.91\% is the final reported test result of the selected hybrid system.
